...
def get_file_from_comfy_output(filename, subfolder="", file_type="output"):
    """
    Fetch file bytes directly from ComfyUI output directory.
    Used for videos/gifs that may not be accessible via /view endpoint.

    Args:
        filename (str): The filename of the file.
        subfolder (str): The subfolder where the file is stored.
        file_type (str): The type folder (e.g., 'output', 'temp').

    Returns:
        bytes: The raw file data, or None if an error occurs.
    """
    # Construct the path to the file in ComfyUI's output directory
    comfy_output_base = os.environ.get("COMFY_OUTPUT_PATH", "/comfyui/output")

    if file_type == "temp":
        comfy_output_base = os.environ.get("COMFY_TEMP_PATH", "/comfyui/temp")

    if subfolder:
        file_path = os.path.join(comfy_output_base, subfolder, filename)
    else:
        file_path = os.path.join(comfy_output_base, filename)

    print(f"worker-comfyui - Fetching file from path: {file_path}")

    try:
        if os.path.exists(file_path):
            with open(file_path, "rb") as f:
                data = f.read()
            print(f"worker-comfyui - Successfully read file {filename} ({len(data)} bytes)")
            return data
        else:
            print(f"worker-comfyui - File not found at {file_path}")
            # Try the /view endpoint as fallback
            return get_image_data(filename, subfolder, file_type)
    except Exception as e:
        print(f"worker-comfyui - Error reading file {filename}: {e}")
        return None
...
def handler(job):
    ...
    try:
        ...
        for node_id, node_output in outputs.items():
            ...
            # Handle GIFs output (from VideoHelperSuite VHS_VideoCombine node)
            if "gifs" in node_output:
                ...
                for gif_info in node_output["gifs"]:
                    ...
                    # Try to get file data - first from filesystem, then via /view endpoint
                    file_bytes = get_file_from_comfy_output(filename, subfolder, file_type)
                    ...
                    if file_bytes:
                        ...
                        if os.environ.get("BUCKET_ENDPOINT_URL"):
                            try:
                                with tempfile.NamedTemporaryFile(
                                    ...
                                    temp_file_path = temp_file.name
                                print(
                                    f"worker-comfyui - Wrote video/gif bytes to temporary file: {temp_file_path}"
                                )
                            ...
                            # Return as base64 string
                            try:
                                base64_video = base64.b64encode(file_bytes).decode(
                                    "utf-8"
                                )
                                output_data.append(
                                    {
                        ...
                        error_msg = f"Failed to fetch video/gif data for {filename}."
                        print(f"worker-comfyui - {error_msg}")
                        errors.append(error_msg)

            # Check for other output types (excluding handled ones)
    ...
    print(f"worker-comfyui - Job completed. Returning {len(output_data)} output file(s).")
    return final_result
